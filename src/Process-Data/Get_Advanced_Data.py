"""Fetch NBA Advanced team stats (OffRtg, DefRtg, NetRtg, Pace, TS%, eFG%, etc.)

Mirrors Get_Data.py but uses MeasureType=Advanced and stores results in
Data/AdvancedTeamData.sqlite with one table per date (same layout).
"""
import argparse
import os
import random
...
DB_PATH = BASE_DIR / "Data" / "AdvancedTeamData.sqlite"
MIN_DELAY_SECONDS = 1
MAX_DELAY_SECONDS = 3
MAX_RETRIES = 3
...
def backfill_season(con, url, season_key, value, existing_dates, today):
    ...
    if not missing_dates:
        print(f"[Advanced] No missing dates for season {season_key}.")
        return

    print(f"[Advanced] Backfilling {len(missing_dates)} dates for season {season_key}.")
    for date_pointer in missing_dates:
        print("[Advanced] Getting data:", date_pointer)
        df = fetch_data(url, date_pointer, value["start_year"], season_key)
        if df.empty:
            print("[Advanced] No data returned for:", date_pointer)
            continue
    ...
    with sqlite3.connect(db_path) as con:
        ...
        if backfill:
            ...
            if season:
                ...
                if not season_items:
                    print("[Advanced] Season not found in config:", season)
                    return
        ...
        if not season_key:
            print("[Advanced] No current season found for today:", today)
            return
        ...
        if fetch_start > fetch_end:
            print("[Advanced] No new dates to fetch. Latest date:", latest_date)
            return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch NBA Advanced team stats data.")
    parser.add_argument(
        "--backfill",
        action="store_true",
